...
const hydrate = nextProps => {
  const columnsFiltered = nextProps.columns.filter(thisColumn => thisColumn.canSearch)
    ...
    : entries.filter(thisEntry => {
        ...
        const results = properties.map(propertySplitted => {
          ...
          const results = Object.keys(thisEntry).map(thisColumn => {
            if (!thisEntry[thisColumn] || !columnsCanSearch.includes(thisColumn)) return false
    ...
    ? entries.sort((a, b) =>
        moment(b[orderBy]).isValid()
          ? moment(b[orderBy]).isBefore(a[orderBy])
            ? -1
            : 1
          : prepareData(b[orderBy]).localeCompare(prepareData(a[orderBy]), undefined, {
              numeric: true,
              sensitivity: 'base',
            })
      )
    : entries.sort((a, b) =>
        moment(a[orderBy]).isValid()
          ? moment(a[orderBy]).isBefore(b[orderBy])
            ? -1
            : 1
          : prepareData(a[orderBy]).localeCompare(prepareData(b[orderBy]), undefined, {
              numeric: true,
              sensitivity: 'base',
            })
      )
...
class EnhancedTable extends Component {
  ...
  getCellContent = (col, entry) => {
    ...
    if (
      ...
      return formatDisplay ? formatDisplay(entry[col.property]) : entry[col.property]
    } else {
  ...
  handleRequestSort = (event, orderBy) => {
    ...
    this.setState({
      ...
      order: this.state.orderBy === orderBy && this.state.order === 'desc' ? 'asc' : 'desc',
      orderBy,
    })
  }
  ...
  render() {
    const { classes, title, columns, keyTableRow, filtersBar, onLoadingDataFromFilter } = this.props
              ...
              <TableBody>
                {((onLoadingDataFromFilter && numberFiltersActive > 0) ||
                  ...
                  data.filter(this.applyFilters).map(entry => (
                    <TableRow key={`${entry._id || this.randomId()}${keyTableRow}`}>
                      {columns
                        ...
                        .map((col, index) => {
                          ...
                          if (
                            col.canTotal === true &&
                            (typeof value === 'number' || typeof parseFloat(value) === 'number')
                          ) {
                          ...
                          return (
                            <TableCell key={index} style={{ textAlign: col.align || 'left' }}>
                              {value}
                            </TableCell>
                          )
                        })}
